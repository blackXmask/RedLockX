...
function simulateHybrid(prompt: string) {
...
function simulateMl(prompt: string) {
  const lower = prompt.toLowerCase()
  const matches = INJECTION_KEYWORDS.filter((kw) => lower.includes(kw))
  if (matches.length === 0) {
...
function decision(hybrid: any, ml: any): FinalVerdict {
...
async function insertLogToSupabase(supabaseUrl: string, serviceKey: string, prompt: string, result: FinalVerdict) {
  try {
    const body = {
      prompt,
    ...
    // ignore logging failures — the API should still return analysis
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // attempt to write to Supabase logs if configured
